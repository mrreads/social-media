  ...
  <div class="content">
    ...
    <div class="dialog-list">
      ...
      { ?>
        ...
        <hr>
      <? } ?>
      <? $count = DB::queryCount("SELECT DISTINCT message_from, user_firstname, user_profileimage FROM `message`, `users` WHERE `users`.id_user = `message`.message_from AND message_to = $idUser UNION SELECT DISTINCT message_to, user_firstname, user_profileimage FROM `message`, `users` WHERE `users`.id_user = `message`.message_to AND message_from = $idUser"); if ($count === 0) { ?> <hr> <p style="width: 100%;text-align: center;"> У вас нет диалогов </p> <? } ?>
    </div>
  </div>
  <script defer>
    let listHR;
